...
java -cp generators/connectors-codegen/target/connectors-codegen-openapi-generator-1.0.0.jar${separator}openapi-generator-cli.jar \
     org.openapitools.codegen.OpenAPIGenerator generate -g connectors-codegen -i camunda.json -o .camunda \
...
for APIFILE in .camunda/element-templates/*.json; do    
    ...
    BASE64=$(source ./fetch-favicon.sh http://www.camunda.com)
...
for APIFILE in .camunda/element-templates/*.json; do
    ...
    if [ $os = "unix" ]; then
        sed -i '' "s@<PREFIX>@CAMUNDA 7 - @g" $APIFILE
    else
        sed -i "s@<PREFIX>@CAMUNDA 7 - @g" $APIFILE
    fi
done